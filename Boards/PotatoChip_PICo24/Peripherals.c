...
#ifndef PICo24_FreeRTOS_Enabled
void __attribute__((interrupt,auto_psv)) _T1Interrupt() {
	IFS0bits.T1IF = 0;

	if (htimer1.InterruptHandler) {
		htimer1.InterruptHandler(htimer1.UserP);
	}
}
#endif
...
void __attribute__((interrupt,auto_psv)) _T2Interrupt() {
	...
	if (htimer2.InterruptHandler) {
		htimer2.InterruptHandler(htimer2.UserP);
	}
}
...
void __attribute__((interrupt,auto_psv)) _T3Interrupt() {
	...
	if (htimer3.InterruptHandler) {
		htimer3.InterruptHandler(htimer3.UserP);
	}
}
...
void __attribute__((interrupt,auto_psv)) _T4Interrupt() {
	...
	if (htimer4.InterruptHandler) {
		htimer4.InterruptHandler(htimer4.UserP);
	}
}
...
void __attribute__((interrupt,auto_psv)) _T5Interrupt() {
	...
	if (htimer5.InterruptHandler) {
		htimer5.InterruptHandler(htimer5.UserP);
	}
}
...
void __attribute__ ((interrupt,auto_psv)) _MI2C3Interrupt() {
