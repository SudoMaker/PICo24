...
bool umm_eds_integrity_check() {
    ...
    prev = 0;
    while (1) {
        cur = UMM_EDS_NFREE(prev);

        /* Check that next free block number is valid */
        if (cur >= UMM_EDS_NUMBLOCKS) {
            UMM_LOG_CRITICAL("Heap integrity broken: too large next free num: %d "
                "(in block %d, addr 0x%08x)\n",
                cur, prev, (&UMM_EDS_NBLOCK(prev)));
            ok = false;
            goto clean;
        }
        if (cur == 0) {
        ...
        if (UMM_EDS_PFREE(cur) != prev) {
            UMM_LOG_CRITICAL("Heap integrity broken: free links don't match: "
                "%d -> %d, but %d -> %d\n",
                prev, cur, cur, UMM_EDS_PFREE(cur));
            ok = false;
            goto clean;
        }

        UMM_EDS_PBLOCK(cur) |= UMM_FREELIST_MASK;
    ...
    prev = 0;
    while (1) {
        cur = UMM_EDS_NBLOCK(prev) & UMM_EDS_BLOCKNO_MASK;

        /* Check that next block number is valid */
        if (cur >= UMM_EDS_NUMBLOCKS) {
            UMM_LOG_CRITICAL("Heap integrity broken: too large next block num: %d "
                "(in block %d, addr 0x%08x)\n",
                cur, prev, (&UMM_EDS_NBLOCK(prev)));
            ok = false;
            goto clean;
        ...
        if ((UMM_EDS_NBLOCK(cur) & UMM_FREELIST_MASK)
            != (UMM_EDS_PBLOCK(cur) & UMM_FREELIST_MASK)) {
            UMM_LOG_CRITICAL("Heap integrity broken: mask wrong at addr 0x%08x: n=0x%x, p=0x%x\n",
                (&UMM_EDS_NBLOCK(cur)),
                (UMM_EDS_NBLOCK(cur) & UMM_FREELIST_MASK),
                (UMM_EDS_PBLOCK(cur) & UMM_FREELIST_MASK));
            ok = false;
            goto clean;
        }
        ...
        if (cur <= prev) {
            UMM_LOG_CRITICAL("Heap integrity broken: next block %d is before prev this one "
                "(in block %d, addr 0x%08x)\n",
                cur, prev, (&UMM_EDS_NBLOCK(prev)));
            ok = false;
            goto clean;
...
/* unmark */
        UMM_EDS_PBLOCK(cur) &= UMM_EDS_BLOCKNO_MASK;

        /* Check if prev block number matches */
        if (UMM_EDS_PBLOCK(cur) != prev) {
            UMM_LOG_CRITICAL("Heap integrity broken: block links don't match: "
                "%d -> %d, but %d -> %d\n",
                prev, cur, cur, UMM_EDS_PBLOCK(cur));
            ok = false;
            goto clean;
        }
