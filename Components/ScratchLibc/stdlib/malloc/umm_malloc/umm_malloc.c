...
static void umm_assimilate_up(uint16_t c) {
	...
	if (UMM_NBLOCK(UMM_NBLOCK(c)) & UMM_FREELIST_MASK) {
		...
		UMM_LOG_DEBUG("umm: Assimilate up to next block, which is FREE\n");
...
void umm_init(void) {
	...
	umm_heap = (umm_block *)UMM_Heap_Address;
	uint16_t blksize = sizeof(umm_block);
	umm_numblocks = UMM_Heap_Size / blksize;
	printf("umm: init: heap=0x%04x, heap_size=%lu, block_size=%u, numblocks=%u\n", umm_heap, UMM_Heap_Size, blksize, umm_numblocks);
	memset(umm_heap, 0x00, UMM_Heap_Size);
...
static void umm_free_core(void *ptr) {
	...
	UMM_LOG_DEBUG("umm: Freeing block %6u\n", c);
	...
	if (UMM_NBLOCK(UMM_PBLOCK(c)) & UMM_FREELIST_MASK) {
		...
		UMM_LOG_DEBUG("umm: Assimilate down to previous block, which is FREE\n");
		...
		UMM_LOG_DEBUG("umm: Just add to head of free list\n");
...
void umm_free(void *ptr) {
	...
	if ((void *)0 == ptr) {
		UMM_LOG_DEBUG("umm: free a null pointer -> do nothing\n");
...
static void *umm_malloc_core(size_t size) {
	...
	while (cf) {
		...
		UMM_LOG_TRACE("umm: Looking at block %6u size %6i\n", cf, blockSize);
	...
	if (UMM_NBLOCK(cf) & UMM_BLOCKNO_MASK && blockSize >= blocks) {
		...
		if (blockSize == blocks) {
			...
			UMM_LOG_DEBUG("umm: Allocating %6i blocks starting at %6u - exact\n", blocks, cf);
			...
			UMM_LOG_DEBUG("umm: Allocating %6i blocks starting at %6u - existing\n", blocks, cf);
		...
		UMM_LOG_DEBUG("umm: Can't allocate %5u blocks\n", blocks);
...
void *umm_malloc(size_t size) {
	...
	if (0 == size) {
		UMM_LOG_DEBUG("umm: malloc a block of 0 bytes -> do nothing\n");
...
void *umm_realloc(void *ptr, size_t size) {
	...
	if (((void *)NULL == ptr)) {
		UMM_LOG_DEBUG("umm: realloc the NULL pointer - call malloc()\n");
	...
	if (0 == size) {
		UMM_LOG_DEBUG("umm: realloc to 0 size, just free the block\n");
	...
	UMM_LOG_DEBUG("umm: realloc blocks %u blockSize %u nextBlockSize %u prevBlockSize %u\n", blocks, blockSize, nextBlockSize, prevBlockSize);
	...
	if (blockSize >= blocks) {
		UMM_LOG_DEBUG("umm: realloc the same or smaller size block - %u, do nothing\n", blocks);
		/* This space intentionally left blank */

		//  Case 2 - block + next block fits EXACTLY
	} else if ((blockSize + nextBlockSize) == blocks) {
		UMM_LOG_DEBUG("umm: exact realloc using next block - %u\n", blocks);
		umm_assimilate_up(c);
		blockSize += nextBlockSize;

		//  Case 3 - prev block NOT free and block + next block fits
	} else if ((0 == prevBlockSize) && (blockSize + nextBlockSize) >= blocks) {
		UMM_LOG_DEBUG("umm: realloc using next block - %u\n", blocks);
		umm_assimilate_up(c);
		blockSize += nextBlockSize;

		//  Case 4 - prev block + block fits
	} else if ((prevBlockSize + blockSize) >= blocks) {
		UMM_LOG_DEBUG("umm: realloc using prev block - %u\n", blocks);
		...
		UMM_LOG_DEBUG("realloc using prev and next block - %u\n", blocks);
		umm_assimilate_up(c);
		umm_disconnect_from_free_list(UMM_PBLOCK(c));
		c = umm_assimilate_down(c, 0);
		memmove((void *)&UMM_DATA(c), ptr, curSize);
		...
		UMM_LOG_DEBUG("umm: realloc a completely new block %u\n", blocks);
		void *oldptr = ptr;
		if ((ptr = umm_malloc_core(size))) {
			UMM_LOG_DEBUG("umm: realloc %u to a bigger block %u, copy, and free the old\n", blockSize, blocks);
			memcpy(ptr, oldptr, curSize);
			umm_free_core(oldptr);
		} else {
			UMM_LOG_DEBUG("umm: realloc %u to a bigger block %u failed - return NULL and leave the old block!\n", blockSize, blocks);
	...
	if (blockSize > blocks) {
		UMM_LOG_DEBUG("umm: split and free %u blocks from %u\n", blocks, blockSize);
