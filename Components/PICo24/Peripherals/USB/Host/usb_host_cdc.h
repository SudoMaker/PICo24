...
typedef struct {
	void (*DeviceConnected)(void *userp, USB_CDC_DEVICE_INFO *device_info);
	void (*DeviceDisconnected)(void *userp, USB_CDC_DEVICE_INFO *device_info);
	void (*RxDone)(void *userp, uint8_t address, const uint8_t *data, uint8_t len);
	void (*TxDone)(void *userp, uint8_t address);
} USBDeluxeHost_CDC_Operations;

typedef struct {
	USBDeluxeHost_CDC_Operations ops;
	void *userp;
} USBDeluxeHost_CDCContext;

extern void USBDeluxe_HostDriver_Enable_CDC(void *userp, USBDeluxeHost_CDC_Operations *cdc_ops);
