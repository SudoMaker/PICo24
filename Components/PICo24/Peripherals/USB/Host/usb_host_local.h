...
typedef struct _USB_DEVICE_INFO {
	...
	uint8_t				numCommandTries;			// The number of times the current command has been tried.
	uint8_t				numEnumerationTries;			// The number of times enumeration has been attempted on the attached device.
	uint8_t				*pCurrentConfigurationDescriptor;	// Pointer to the current configuration descriptor of the attached device.
	uint8_t				*pDeviceDescriptor;			// Pointer to the Device Descriptor of the attached device.
	uint8_t				*pEP0Data;				// A data buffer for use by EP0.

	uint16_t			numTimerInterrupts;			// The number of milliseconds elapsed during the current waiting period.
	USB_ENDPOINT_INFO		*pCurrentEndpoint;			// Pointer to the endpoint currently performing a transfer.
	uint16_t			usbHostState;				// State machine state of the attached device.
	uint16_t			usbOverrideHostState;			// Next state machine state, when set by interrupt processing.
	uint16_t			usbLastHostState;				// State machine state of the attached device.
} USB_DEVICE_INFO;
...
#define _USB_InitErrorCounters(dev)        { dev->numCommandTries   = USB_NUM_COMMAND_TRIES; }
#define _USB_SetDATA01(dev, x)               { dev->pCurrentEndpoint->status.bfNextDATA01 = x; }
#define _USB_SetErrorCode(dev, x)            { dev->errorCode = x; }
#define _USB_SetHoldState(dev)             { dev->usbHostState = STATE_HOLDING; }
#define _USB_SetNextState(dev)             { dev->usbHostState = (dev->usbHostState & STATE_MASK) + NEXT_STATE; }
#define _USB_SetNextSubState(dev)          { dev->usbHostState = (dev->usbHostState & (STATE_MASK | SUBSTATE_MASK)) + NEXT_SUBSTATE; }
#define _USB_SetNextSubSubState(dev)       { dev->usbHostState += NEXT_SUBSUBSTATE; }
#define _USB_SetNextTransferState()     { dev->pCurrentEndpoint->transferState ++; }
#define _USB_SetPreviousSubSubState()   { dev->usbHostState =  dev->usbHostState - NEXT_SUBSUBSTATE; }
#define _USB_SetTransferErrorState(x)   { x->transferState = (x->transferState & TSTATE_MASK) | TSUBSTATE_ERROR; }

extern void USBHostConfigureDeviceSlot(uint8_t slot_index, bool enabled, bool low_speed);
extern ssize_t USBHostGetUnusedDeviceSlot();
extern USB_DEVICE_INFO *USBHostGetDeviceInfoBySlotIndex(uint8_t slot_index);
extern USB_DEVICE_INFO *USBHostGetDeviceInfoOfFirstDevice();
...
void                 _USB_SetBDT(USB_DEVICE_INFO *dev, uint8_t  direction);
