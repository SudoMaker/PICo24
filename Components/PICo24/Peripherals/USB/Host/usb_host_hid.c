...
#define HID_STATE_DETACHED                      0x0000 //

#define HID_STATE_INITIALIZE_DEVICE             0x0001 //
#define HID_STATE_WAIT_FOR_REPORT_DSC           0x0002 //
#define HID_STATE_PARSING_COMPLETE              0x0003 //

#define HID_STATE_RUNNING                       0x0004 //

#define HID_STATE_WAIT_FOR_RESET                0x0007 //
#define HID_STATE_RESET_COMPLETE                0x0008 //

#define HID_STATE_HOLDING                       0x0009 //

//------------------------------------------------------------------------------
// Constants for the the individual HID request (read or write)

#define HID_STATE_TRANSFER_WAITING                  0
#define HID_STATE_TRANSFER_REQUESTED                1
#define HID_STATE_TRANSFER_COMPLETE                 2
...
#define _USBHostHID_LockDevice(x)                   {                                                   \
                                                        ...
                                                        deviceInfoHID[i].state      = HID_STATE_HOLDING;    \
                                                    }

#ifdef USB_HID_ENABLE_TRANSFER_EVENT
#define _USBHostHID_TerminateReadTransfer( error )  {                                                                                   \
                                                            deviceInfoHID[i].errorCode          = error;                                    \
                                                            deviceInfoHID[i].state              = HID_STATE_RUNNING;                            \
                                                            deviceInfoHID[i].transferIN.state   = HID_STATE_TRANSFER_WAITING;                   \
    ...
    #define _USBHostHID_TerminateWriteTransfer( error ) {                                                                                   \
                                                            deviceInfoHID[i].errorCode          = error;                                    \
                                                            deviceInfoHID[i].state              = HID_STATE_RUNNING;                            \
                                                            deviceInfoHID[i].transferOUT.state  = HID_STATE_TRANSFER_WAITING;                   \
                                                            USB_HOST_APP_EVENT_HANDLER( deviceInfoHID[i].ID.deviceAddress,                  \
...
uint8_t    USBHostHIDDeviceStatus( uint8_t deviceAddress )
{
	...
	else
	{
		...
		{
			case HID_STATE_INITIALIZE_DEVICE:
			case HID_STATE_PARSING_COMPLETE:
			...
			case HID_STATE_HOLDING:
				return USB_HID_DEVICE_HOLDING;
				break;

			case HID_STATE_WAIT_FOR_RESET:
			case HID_STATE_RESET_COMPLETE:
			...
			uint8_t size, uint8_t *data )
{
	...
	if ( ! ( ( deviceInfoHID[i].state == HID_STATE_RUNNING ) &&
		 ( deviceInfoHID[i].transferIN.state == HID_STATE_TRANSFER_WAITING ) ) )
	{
	...
	if (!errorCode)
	{
		deviceInfoHID[i].transferIN.state  = HID_STATE_TRANSFER_REQUESTED;
	}
	else
	{
...
bool USBHostHIDReadIsComplete ( uint8_t deviceAddress, uint8_t *errorCode, uint8_t *byteCount )
{
	...
	// Find the correct device.
	for (i=0; (i<USB_MAX_HID_DEVICES) && (deviceInfoHID[i].ID.deviceAddress != deviceAddress); i++);
	if ((i == USB_MAX_HID_DEVICES) || (deviceInfoHID[i].state == HID_STATE_DETACHED))
	{
	...
	*byteCount = deviceInfoHID[i].transferIN.bytesTransferred;
	...
	if(deviceInfoHID[i].transferIN.state == HID_STATE_TRANSFER_WAITING)
	{
		return true;
	}
	else
...
uint8_t USBHostHIDResetDevice( uint8_t deviceAddress )
{
	...
	if ((deviceInfoHID[i].state != HID_STATE_DETACHED) &&
	    (deviceInfoHID[i].state != HID_STATE_INITIALIZE_DEVICE))
	{
		deviceInfoHID[i].flags.val |= MARK_RESET_RECOVERY;
		deviceInfoHID[i].flags.bfReset = 1;
		deviceInfoHID[i].returnState = HID_STATE_RUNNING;
			 ...
			 uint8_t size, uint8_t *data )
{
	...
	if (!errorCode)
	{
		deviceInfoHID[i].transferOUT.state  = HID_STATE_TRANSFER_REQUESTED;
	}
	else
	{
		deviceInfoHID[i].errorCode          = errorCode;
...
bool USBHostHIDWriteIsComplete ( uint8_t deviceAddress, uint8_t *errorCode, uint8_t *byteCount )
{
	...
	*byteCount = deviceInfoHID[i].transferOUT.bytesTransferred;
	...
	if(deviceInfoHID[i].transferOUT.state == HID_STATE_TRANSFER_WAITING)
	{
		return true;
	}
	else
...
//	printf("USBHostHIDEventHandler: addr=%u, event=%d, data=%p, size=%lu\n", address, event, data, size);
	...
	switch (event)
	{
		...
		case EVENT_DETACH:           // USB cable has been detached (data: uint8_t, address of device)
			...
			if (i < USB_MAX_HID_DEVICES)
			{
				...
				deviceInfoHID[i].ID.deviceAddress   = 0;
				deviceInfoHID[i].state              = HID_STATE_DETACHED;
			}
		...
		case EVENT_TRANSFER:         // A USB transfer has completed
			...
			switch (deviceInfoHID[i].state)
			{
				case HID_STATE_WAIT_FOR_REPORT_DSC:
#ifdef DEBUG_MODE
...
//                             deviceInfoHID[i].bytesTransferred = ((HOST_TRANSFER_DATA *)data)->dataCount;
						...
						else
						{
...
#endif
									printf("HID: Error parsing descriptor\n");
							...
							if(pCurrInterfaceDetails != NULL)
							{
								...
								deviceInfoHID[i].state = HID_STATE_WAIT_FOR_REPORT_DSC;
							}
							else
							{
								if(deviceInfoHID[i].flags.bfReportDataCollected == 0)
								{
									...
									printf( "HID: Problem collecting report data\n" );
									_USBHostHID_FreeRptDecriptorDataMem(deviceInfoHID[i].ID.deviceAddress);
									_USBHostHID_LockDevice( USB_HID_REPORT_DESCRIPTOR_BAD );
									USB_HOST_APP_EVENT_HANDLER(deviceInfoHID[i].ID.deviceAddress, EVENT_HID_BAD_REPORT_DESCRIPTOR, NULL, 0);
								}
				...
				case HID_STATE_RUNNING:
					if ( ( deviceInfoHID[i].transferIN.state == HID_STATE_TRANSFER_REQUESTED ) &&
					...
					else if ( (deviceInfoHID[i].transferOUT.state == HID_STATE_TRANSFER_REQUESTED ) &&
...
bool USBHostHIDInitialize(uint8_t address, uint32_t flags, uint8_t clientDriverID) {
	...
	descriptor = dev->pDeviceDescriptor;
...
//    }
	...
	if(validConfiguration)
	{
		...
		deviceInfoHID[device].state             = HID_STATE_WAIT_FOR_REPORT_DSC;

		deviceInfoHID[device].transferIN.state  = HID_STATE_TRANSFER_WAITING;
		deviceInfoHID[device].transferOUT.state = HID_STATE_TRANSFER_WAITING;
...
static void _USBHostHID_ResetStateJump( uint8_t i )
{
	...
	if (deviceInfoHID[i].flags.bfReset)
	{
		...
		{
			deviceInfoHID[i].state = HID_STATE_WAIT_FOR_RESET;
		}
	}
	else
	{
