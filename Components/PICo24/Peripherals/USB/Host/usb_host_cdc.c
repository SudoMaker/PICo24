...
#define CDC_STATE_RUNNING                             0x0020 //
...
#define CDC_STATE_HOLDING                             0x0040 //
...
#define CDC_STATE_RUNNING                             0x0005 //
...
#define CDC_STATE_HOLDING                             0x000A //
...
// Default values for RS232 configuration data
    ...
    #define USB_CDC_BAUDRATE_SUPPORTED 921600L // default BAUDRATE for the device
#endif
    ...
    #define USB_CDC_STOP_BITS 1           // default Stop bits
#endif
...
#define _USBHostCDC_TerminateTransfer( error )  {                                                                       \
                                                    ...
                                                    deviceInfoCDC[i].state        = CDC_STATE_RUNNING;\
                                                }
...
USBDeluxeHost_CDCContext cdc_host_ctx = {0};
...
uint8_t USBHostCDCResetDevice( uint8_t deviceAddress )
{
    ...
    if (i == USB_MAX_CDC_DEVICES)
    {
       ...
       if (((deviceInfoCDC[i].state & STATE_MASK) != STATE_DETACHED) &&
        ...
        #ifndef USB_ENABLE_TRANSFER_EVENT
            deviceInfoCDC[i].returnState = CDC_STATE_HOLDING;
        #else
            deviceInfoCDC[i].returnState = CDC_STATE_RUNNING;
        #endif
                              ...
                              if (USBHostTransferIsComplete( deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA, &errorCode, &byteCount ))
                              {
                                if (errorCode)
                                {
                                    ...
                                    }else if(USB_ENDPOINT_NAK_TIMEOUT == errorCode)
                                    {
                                        ...
                                        deviceInfoCDC[i].state = CDC_STATE_RUNNING | SUBSTATE_WAITING_FOR_REQ;
                                        USBHostClearEndpointErrors( deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA );
                                    }
                                    else
                                    {
                              ...
                              if (USBHostTransferIsComplete( deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA, &errorCode, &byteCount ))
                              {
                                ...
                                else
                                {
                                    ...
                                    if(deviceInfoCDC[i].remainingBytes != 0)
                                    {
                                        ...
                                        deviceInfoCDC[i].state = CDC_STATE_RUNNING | SUBSTATE_SEND_WRITE_REQ;
                                    }
                                    else
                                    {
...
uint8_t USBHostCDCTransfer( uint8_t deviceAddress,uint8_t request , uint8_t direction, uint8_t interfaceNum, uint16_t size, uint8_t *data , uint8_t endpointDATA)
{
    ...
    #ifndef USB_ENABLE_TRANSFER_EVENT
        if (deviceInfoCDC[i].state != (CDC_STATE_RUNNING) &&
    ...
    deviceInfoCDC[i].commRequest       = request;       // invalid entry if DATA transfer is requested
        ...
        if(!direction)
            ...
            {
                deviceInfoCDC[i].state             = CDC_STATE_RUNNING | SUBSTATE_SEND_READ_REQ;
            }
    #else
        USBHostClearEndpointErrors( deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA );
        if(!direction)
...
bool USBHostCDCTransferIsComplete( uint8_t deviceAddress, uint8_t *errorCode, uint8_t *byteCount )
{
    ...
    *errorCode = deviceInfoCDC[i].errorCode;
      ...
      #ifndef USB_ENABLE_TRANSFER_EVENT
            if(deviceInfoCDC[i].state == (CDC_STATE_RUNNING | SUBSTATE_WAITING_FOR_REQ))
      #else
            if(deviceInfoCDC[i].state == CDC_STATE_RUNNING)
      #endif
                {
...
bool USBHostCDCEventHandler( uint8_t address, USB_EVENT event, void *data, uint32_t size )
{
    ...
    switch (event)
    {
        ...
        case EVENT_DETACH:           // USB cable has been detached (data: uint8_t, address of device)
            ...
            if (i < USB_MAX_CDC_DEVICES)
            {
		    if (cdc_host_ctx.ops.DeviceDisconnected) {
			    cdc_host_ctx.ops.DeviceDisconnected(cdc_host_ctx.userp, &deviceInfoCDC[i]);
		    }
        ...
        case EVENT_TRANSFER:         // A USB transfer has completed - NOT USED
		    ...
		    printf("deviceInfoCDC[i].state = 0x%04x\n", deviceInfoCDC[i].state);
                switch (deviceInfoCDC[i].state)
                {
					...
					printf("deviceInfoCDC A\n");
					    ...
					    // if fine goto set  Control line state
                                    ...
                                    {
					    printf("deviceInfoCDC C\n");

					    deviceInfoCDC[i].state     = CDC_STATE_RUNNING;

					    if (cdc_host_ctx.ops.DeviceConnected) {
						    cdc_host_ctx.ops.DeviceConnected(cdc_host_ctx.userp, &deviceInfoCDC[i]);
					    }
...
//				       }
                    ...
                    case STATE_WRITE_REQ_WAIT :
                              if (USBHostTransferIsComplete( deviceInfoCDC[i].deviceAddress, deviceInfoCDC[i].endpointDATA, &errorCode, &byteCount ))
                              {
				      printf("STATE_WRITE_REQ_WAIT ec: 0x%04x\n", errorCode);
...
bool USBHostCDCInitialize( uint8_t address, uint32_t flags, uint8_t clientDriverID )
{
	...
	descriptor = dev->pCurrentConfigurationDescriptor;
	...
	// Find the next interface descriptor.
		...
		hexdump(descriptor, ((USB_CONFIGURATION_DESCRIPTOR *)descriptor)->wTotalLength);
           ...
           i += descriptor[i];
		printf("USBHostCDCInitialize x descriptor[i]=%u\n", descriptor[i]);
	...
	printf("USBHostCDCInitialize done\n");
...
*******************************************************************************/
void USBHostCDC_Init_CDC_Buffers() {
   CDC_LINE_CODING_Buffer.dwDTERate = 115200;
   CDC_LINE_CODING_Buffer.bCharFormat = 1;
   CDC_LINE_CODING_Buffer.bParityType = 0;
   CDC_LINE_CODING_Buffer.bDataBits   = 8;
}

void USBDeluxe_HostDriver_Enable_CDC(void *userp, USBDeluxeHost_CDC_Operations *cdc_ops) {
	memcpy(&cdc_host_ctx.ops, cdc_ops, sizeof(USBDeluxeHost_CDC_Operations));

	uint16_t host_cdc = USBDeluxe_Host_AddDriver(USB_FUNC_CDC_ACM, USBHostCDCInitialize, USBHostCDCEventHandler, NULL, 0);

	USBDeluxe_Host_AddDeviceClass(host_cdc, 0, 2, 2, 1);
	USBDeluxe_Host_AddDeviceClass(host_cdc, 0, 0x0a, 0, 0);
}