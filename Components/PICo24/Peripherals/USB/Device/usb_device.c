...
// *****************************************************************************
// *****************************************************************************
extern bool USB_Device_EventHandler(USB_EVENT event, void *pdata, uint16_t size);
	...
	else
	{
		...
		if(outPipes[0].pFunc != NULL)
		{
			...
			//Special pragmas to suppress an expected/harmless warning
                ...
                #pragma warning pop
#else
			outPipes[0].pFunc(outPipes[0].pFuncUserP);    //Call the user's callback function
#endif
		}
...
//				printf("str desc req: %u\n", SetupPkt.bDscIndex);

				if (SetupPkt.bDscIndex < usb_device_string_descriptor_table.size) {
					//Get a pointer to the String descriptor requested
					uint8_t *pstr = *(void **)Vector_At(&usb_device_string_descriptor_table, SetupPkt.bDscIndex);
//					printf("str desc %u: addr 0x%04x len %u\n", SetupPkt.bDscIndex, pstr, *pstr);
					inPipes[0].pSrc.bRom = pstr;
					// Set data count
...
static void USBCtrlTrfInHandler(void)
{
	...
	else // must have been a CTRL_TRF_RX status stage IN packet (<setup><out><out>...<IN>  <-- this last IN just occurred as the status stage)
	{
		...
		if(outPipes[0].info.bits.busy == 1)
		{
			if(outPipes[0].pFunc != NULL)
			{
				outPipes[0].pFunc(outPipes[0].pFuncUserP);
			}
			outPipes[0].info.bits.busy = 0;
		}
